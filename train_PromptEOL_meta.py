...
def load_dataset_for_train(dataset_name, batch_size):
    # データの準備
    if dataset_name == "qqp":
    ...
    with open(f"./pickle_datas/{dataset_name}-Etype59plus-kai5-train.pickle", mode='br') as fi:
        dataset_train = pickle.load(fi)
    with open(f"./pickle_datas/{dataset_name}-Etype59plus-kai5-validation{validation_split_tag}.pickle", mode='br') as fi:
    ...
    return dataset_source, train_loader, val_loader


def trainer(device, variant_model_name, save_name, dataset_name, hyper_params, base_model_name):
    epoch_size = hyper_params["epoch_size"]
    batch_size = hyper_params["batch_size"]
    lr = hyper_params["lr"]

    dataset_source, train_loader, val_loader = load_dataset_for_train(dataset_name, batch_size)
    ...
    update_param_names = ["meta_fc.0.weight", "meta_fc.0.bias", "meta_fc.2.weight", "meta_fc.2.bias"]
...
if __name__ == "__main__":
    ...
    variant_model_names = [
        ...
        "PromptEolMetaModelSimpleConcat",
        "PromptEolMetaModelConcatAsub"
    ]
    ...
    for i in range(num_of_model):
        ...
        save_name = variant_model_name + f"_{base_model_name}_model_trained_" + dataset_name + "-kai-OnKai5" + save_date
        trainer(device, variant_model_name, save_name, dataset_name, hyper_params, base_model_name )
